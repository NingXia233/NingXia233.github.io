...
其中$P=[P_1,P_2,..,P_N],Q=[Q_1,Q_2,...,Q_N]$是$1,2,..,N$的两组置换，$A[Q,P]$为叠加系数矩阵，并记置换$P$所对应的列向量为$\xi_P$。为了考虑相互作用的影响，我们考虑以下两个区域的波函数：$Q=[Q_1,Q_2,Q_3,Q_4,...,Q_N],Q'=[Q'_1,Q'_2,Q'_3,Q'_4,...,Q'_N]=[Q_1,Q_2,Q_4,Q_3,...,Q_N]$。于是在$x_{Q_3}=x_{Q_4}$的边界处，波函数需要满足波函数的连续性条件，即：

$$
\sum_{P}A[Q,P]\exp(i\sum_k p_{P_k}x_{Q_k}) = \sum_{P}A[Q',P]\exp(i\sum_k p_{P_k}x_{Q'_k})
$$

注意到$Q_3=Q'_4, Q_4=Q'_3$，于是我们考虑将$P$的求和分成两类，分别记作$P=[P_1,P_2,P_3,P_4,...,P_N],P'=[P_1,P_2,P_4,P_3,...,P_N]$，于是得到：

$$
\begin{aligned}
&\sum_{P}(A[Q,P]-A[Q',P'])\exp(i[...+p_{P_3}x_{Q_3}+p_{P_4}x_{Q_4}+...])=\\
&\sum_{P}(A[Q',P]-A[Q,P'])\exp(i[...+p_{P_4}x_{Q_3}+p_{P_4}x_{Q_4}+...])
\end{aligned}
$$

在$x_{Q_3}=x_{Q_4}$的条件下，我们得到：$A[Q,P]+A[Q,P']=A[Q',P]+A[Q',P']$。

此外，波函数还需要在边界处满足导数的跳变条件，考虑对薛定谔方程在边界处积分：

$$
\int_{0^-}^{0^+}d(x_{Q_3}-x_{Q_4})\bigg[-\sum_i \frac{\partial^2\psi}{\partial x_i^2}+c\delta(x_{Q_3}-x_{Q_4})\psi\bigg]=0
$$

进一步处理得到：

$$
\bigg(\frac{\partial\psi}{\partial x_{Q_3}}-\frac{\partial\psi}{\partial x_{Q_4}}\bigg)\bigg|_{0^-}^{0^+}=c\psi(x_1,...,x_N|x_{Q_3}=x_{Q_4})
$$
