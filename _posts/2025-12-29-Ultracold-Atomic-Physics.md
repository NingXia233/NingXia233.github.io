...
- 在$\lvert ka_s\rvert\ll 1$时，$\lvert a_s\rvert$的强弱在一定程度上反应了相互作用的强弱，此时散射截面正比于$\lvert a_s\rvert^2$，而散射长度发散时（散射共振），散射截面正比于动量$k$（幺正气体,unitary gas）
...
对于均匀系统$V(r)=0$，场方程的基态有$\mu=Un_0,n_0=\lvert\psi\rvert^2$
...
如果$ \lvert v_i\rvert < c$，那么此时方程无法得到满足，杂质将不会被散射，这就是超流临界速度。更一般地，考虑低能激发模式为$\mathcal{E}(q)$，那么临界速度将变成:
...
由于子格点对称性，系统的能带会分成上下两支$\pm\lvert B(k)\rvert$，只要$\lvert B(k)\rvert$在任何$k$都不为零，两支能带就不会相交。在这种情况下我们就可以定义:
...
在Bose Hubbard模型中，有两个重要参数$\mu/U, J/U$：

在$J/U\ll 1$的极限下，我们有莫特绝缘体(引入粒子/空穴激发需要至少克服一个有限大的能隙)，改变$\mu/U$可以引发不同填充数之间的相变（在J=0时为一级相变）

固定$\mu/U$，改变$J/U$时，会发生从莫特绝缘体到超流的相变，我们可以用平均场近似来处理，其中序参量$\phi\sim\langle b\rangle$。

相变对应于破缺了连续$U(1)$对称性的GL相变。在平均场的基础上引入相位和振幅涨落，可以得到无能隙的Goldstone模式和有能隙的Higgs模式。

## Fermi Hubbard模型

在费米Hubbard模型中，我们已知的有以下几个重要性质：

- 可以通过粒子-空穴变换的复合将$-U$与$+U$的Hubbard模型联系起来。超导和反铁磁也可以在变换中互相对应。
- 可以引入$h,\mu$作为坐标轴，或者正则系综中以$N_\uparrow-N_\downarrow,N_\uparrow+N_\downarrow-N$作为坐标轴，相图中的点性质可以由此通过粒子-空穴变换复合互相映射。
- 在U<0时，基态favor双占据同时引入空位，因此在给出超导的同时也会带来电荷密度涨落Charge Density Wave。
- 在U>0时，基态favor单占据，通过粒子空穴映射复合，可以把Charge Density Wave序参量映射为面外反铁磁有序，而超导序参量则可以映射成面内反铁磁有序。
- U>0时的反铁磁有序还有两个不同的出发点，在巡游图像下，我们把相互作用进行平均场处理，Nesting会导致反铁磁的自旋密度波；在局域图像下，我们把Hopping作微扰处理，超交换机制导致反铁磁有序。
- 在粒子-空穴复合变换下，在$h=\mu=0$处,系统具有最高的对称性$SU(2)\times SU(2)/Z_2=SO(4)$
